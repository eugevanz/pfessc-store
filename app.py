...
app.jinja_env.filters['to_html'] = lambda text: BeautifulSoup(text, 'html.parser')

url: str = os.environ.get("SUPABASE_URL")
key: str = os.environ.get("SUPABASE_KEY")
supabase: Client = create_client(url, key)

products = supabase.table("Products").select("fullCode", "productName", "brand", "images", "variants", "keywords", "description").execute()
...
def shopall():
    ...
    products = supabase.table("Products").select("simpleCode", "productName", "brand", "images").range(start_index, start_index + 10).execute()
    return render_template("products.html", products=products.data, page=page)

@app.route("/add-to-cart/<item_id>")
def add_to_cart(item_id):
    item = dict({"name": item_id, "image": "product.image", "price": "product.price", "quantity": 1, "size": "L", "color": "grey"})
...
def buy_now():
    ...
    item = dict({"name": item_name, "image": item_image, "price": item_price, "quantity": selected_quantity, "size": selected_size, "color": selected_color})
...
def item_desc(fullCode):
    desc = next((product.get("description") for product in products.data if product.get("fullCode") == fullCode), None)
    print(desc)
    return desc